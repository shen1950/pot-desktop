...
import { MdEdit, MdCheck, MdClose, MdContentCopy } from 'react-icons/md';
...
export default function MessageList({ messages, isLoading, onEditConfirm, onRegenerate, onSystemPromptChange }) {
    ...
    return (
        <div className='flex-1 overflow-y-auto p-3 space-y-3'>
            ...
            {/* Empty state after clearing the conversation */}
            {displayMessages.length === 0 && !isLoading && (
                <div className='h-full flex items-center justify-center text-default-400 text-sm select-none'>
                    {t('chat.empty')}
                </div>
            )}
                ...
                return (
                    <div key={idx}>
                        <div className={`group flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                            ...
                            {/* Action buttons — assistant messages, right side */}
                            {!isUser && !isLoading && !isEditing && (
                                <div className='self-center ml-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity'>
